...
class Server {
	...
	startServer(app) {
		 const options = {
			key: fs.readFileSync(path.join(__dirname, '/certs/server.key')), // Adjust path as needed
			cert: fs.readFileSync(path.join(__dirname, '/certs/server.cert')) // Adjust path as needed
		};
		...
		return new Promise((resolve, reject) => {
			...
			this.server.listen(this.port, () => {
				...
				logger.info(`Mocking API at https://localhost:${realPort}/`);
				resolve();
			});
		});
	}
